...
int main(int argCount, char **argValues) {
	...
	// Inits Window were to draw
	if (!glfwos.InitializeWindow(1024, 768, "Trillek")) {
	...
	// Checking if OpenGL initiated and version
	if (version[0] == -1) {
		LOG_ERROR << "Error starting OpenGL!";
		return -1;
	}
	else if (version[0] < 3) {
		LOG_ERROR << "Trillek needs OpenGL 3!";
		return -1;
	}
	else {
	...
	LOG << "Initializing Virtual Computer system.";
	vcsys.Start();
	...
	{ // FIXME This crash on Linux sometimes
		/*
