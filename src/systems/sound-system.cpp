...
void System::HandleEvents(const frame_tp& timepoint) {
    auto transform_history = TransformMap::GetAsyncUpdatedTransforms().GetHistoryData(timepoint, last_received_frame);
    for(auto itmap = transform_history.cbegin(); itmap != transform_history.cend(); ++itmap) {
    ...
    if (transform_history.size() > 1) {
        std::cerr << "Sound system was " << transform_history.size()- 1 << " frame(s) late" << std::endl;
    }
}
